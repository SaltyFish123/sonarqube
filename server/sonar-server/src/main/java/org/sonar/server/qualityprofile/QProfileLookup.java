...
public class QProfileLookup {
  ...
  public List<QualityProfileDto> profiles(DbSession dbSession, String language, OrganizationDto organization) {
    return db.qualityProfileDao().selectByLanguage(dbSession, language);
  }

  @CheckForNull
  public QualityProfileDto profile(String name, String language) {
    try (DbSession dbSession = db.openSession(false)) {
      return findQualityProfile(name, language, dbSession);
    }
  }
